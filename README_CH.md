...
> 由于该apk的SDK版本实在是太老了，因此在最新的Android系统上可能会被拒绝安装。
>
> 遇到这种情况，请给Android设备启用ADB调试，然后连接到PC，通过ADB命令安装：
>
>> `adb install <apk文件路径>`
>
> 如果还是安装失败，加上 `--bypass-low-target-sdk-block` 参数再试试：
>
>> `adb install --bypass-low-target-sdk-block <apk文件路径>`
